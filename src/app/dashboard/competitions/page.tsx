...
import { useRouter } from 'next/navigation'; // ✅ Use Next Router instead of window.location
...
export default function CompetitionsListPage() {
  ...
  const handleJoin = async (comp: any) => {
    ...
    setJoiningId(comp.id); // Show loading state on specific button

    try {
      const res: any = await api.post(`/competitions/${comp.id}/join`);
      
      // ✅ 1. Update local state immediately so UI updates
      setCompetitions(prev => prev.map(c => {
        if (c.id === comp.id) {
          return {
            ...c,
            participants: [...(c.participants || []), user] // Add current user to list
          };
        }
        return c;
      }));
      ...
      // ✅ 2. Use router.push for smooth internal navigation
      const firstAssetId = res.firstAssetId || comp.allowedAssets?.[0]?.id || 1;
